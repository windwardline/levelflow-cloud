...
describe("the Target 2 refusal names its cause (round-8 PH-9)", () => {
  // The gate reads Target 2 on a full-size position. The Desk prints the
  // ladder's payoff, which is smaller because half the position leaves at
  // TP1, so the refusal may not call its own figure "payoff".
  it("translates the cost-driven refusal distinctly from the geometry one", () => {
    assert.equal(
      cleanReviewMessage(
        "Trading costs took Target 2 on the full position from 2.10x to 1.18x the risk; Levelflow requires at least 1.25x for this market.",
      ),
      "Trading costs took Target 2 on the full position from 2.10x to 1.18x (1.25x required).",
    );
    assert.equal(
      cleanReviewMessage(
        "Target 2 on the full position would pay 1.10x the risk; Levelflow requires at least 1.25x for this market.",
      ),
      "Target 2 was not far enough from the entry to justify the risk (1.10x on the full position; 1.25x required).",
    );
  });
