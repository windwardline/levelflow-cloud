 ...
 *
 * Each test runs a copy of the real wrapper in which only the `REPO=` line
 * differs, with `npx` and `security` replaced by stubs on PATH. The stub `npx`
 * logs its arguments and plays the gate, the driver and the marker writer, so
 * a test states what each step printed and returned and reads what the
 * wrapper did with it.
 *
...
function sandbox(wrapper: string) {
  const root = mkdtempSync(join(tmpdir(), "ops-wrapper-"));
  const repo = join(root, "repo");
  const bin = join(root, "bin");
  mkdirSync(join(repo, ".minute-bank"), { recursive: true });
  mkdirSync(bin);
  const source = readFileSync(wrapper, "utf8");
  const copy = source.replace(/^REPO=".*"$/m, `REPO="${repo}"`);
  const before = source.split("\n");
  const after = copy.split("\n");
  assert.equal(before.length, after.length);
  assert.equal(
    before.filter((line, at) => line !== after[at]).length,
    1,
    "exactly the REPO= line differs from the real wrapper",
  );
  const script = join(root, "wrapper.sh");
  writeFileSync(script, copy);
...
function run(wrapper: string, steps: { gate?: Step; driver?: Step; record?: Step }) {
  ...
  return { calls, code: result.status, output: `${result.stdout}${result.stderr}`, repo: box.repo };
}
...
describe("the nightly top-up wrapper, executed", () => {
  it("skips on the gate's 75 and its skip line, before the Keychain or the driver", () => {
    ...
    assert.deepEqual(result.calls, ["npx tsx scripts/fmpRunGate.ts --job cache-topup"]);
...
describe("the minute-bank wrapper, executed", () => {
  ...
  it("runs the bank into the repository's store", () => {
    const result = run(BANK, {});
    assert.equal(result.code, 0, result.output);
    assert.equal(result.calls[0], `npx tsx scripts/fmpRunGate.ts --job minute-bank --dir ${result.repo}/.minute-bank`);
    assert.deepEqual(driverCalls(result.calls), [`npx tsx scripts/bank-minute-bars.ts --dir ${result.repo}/.minute-bank`]);
    assert.ok(existsSync(join(result.repo, ".minute-bank")));
  });
