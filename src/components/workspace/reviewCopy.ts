export function cleanReviewMessage(value: string) {
  return value
    ...
    .replace(/Correlation filter kept existing ([A-Z0-9]+) setup with equal or higher confidence\./i, (_match, symbol: string) => `A stronger related setup is already active on ${symbol}.`)
    .replace(/A stronger related-market setup is already active on ([A-Z0-9]+)\./i, (_match, symbol: string) => `A stronger related setup is already active on ${symbol}.`)
