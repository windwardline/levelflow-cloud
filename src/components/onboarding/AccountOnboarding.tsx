...
import { BadgeDollarSign, CheckCircle2, LockKeyhole, RadioTower, Save, SlidersHorizontal } from "lucide-react";
    ...
    <form className="grid gap-5 lg:grid-cols-[1.08fr_0.92fr]" onSubmit={saveAccount}>
      ...
      <aside className="space-y-5">
        ...
        <section className="terminal-panel p-5 sm:p-6">
          ...
          <button className="primary-button mt-5 w-full" type="submit" disabled={status === "saving"}>
            {status === "saved" ? <CheckCircle2 className="h-4 w-4" aria-hidden="true" /> : <Save className="h-4 w-4" aria-hidden="true" />}
            {status === "saving" ? "Saving..." : status === "saved" ? "Configuration saved" : "Save account"}
          </button>
