...
rc=0
rclone lsf -R --files-only "R2:$BUCKET/$PREFIX/" --include '*.tar.zst' >"$WORK/listing" 2>"$WORK/lsf.err" || rc=$?
if [[ $rc -eq 3 ]]; then
...
# days. Any *.tar.zst outside it is a finding, named rather than skipped: the
# push counts and prunes this prefix by the same filter, so a misnamed archive
# here is in its population whether or not it is in this one.
