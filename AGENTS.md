...
- **A pinned job names its checkout, and no test reaches the provider.** Jobs run through `wl-repo-script` get `origin/main`'s code and none of the checkout's ignored data, so anything touching FMP must set `LEVELFLOW_CHECKOUT`: the usage ledger and circuit marker resolve through `scripts/checkoutState.ts`, and unset they read as untouched from a directory deleted on exit. The minute bank has run this way since 2026-09-20; `docs/minute-bank.md` lists what each piece of data would otherwise have done. A test that executes a script which reads the keychain runs with `tests/support/noKeychain.ts` in its environment. On 2026-09-20 four runs without it fetched a full FMP roster each into a sandbox.
