...
- **The minute bank is proven by restoring it.** `scripts/ops/verify-minute-bank-restore.sh` restores the newest R2 archive, refuses one more than three days old, and requires every restored file to be a byte prefix of the live bank and every live symbol older than the snapshot to be in it; parity compares names and the push one md5, so neither is evidence that an archive restores. It is to be run weekly beside the Postgres proof; the `CADENCE.md` row that would schedule it is still owed in windwardline/windwardline.
