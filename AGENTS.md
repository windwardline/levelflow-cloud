...
- **Every argv reader refuses an unknown flag by name, and the population is derived.** `tests/unknownFlagRefused.test.ts` finds each reader by what it reads (ten spellings of an argv read, pinned), executes it with an unknown flag and a stray argument, and requires every flag it reads to be declared; the shared walk is `positionalArgs` and `flagsOnly` in `scripts/flagReader.ts`. `STACK_ON_REFUSAL` names the thirteen readers that still print a stack on an operator's typo, and it only shrinks: a fixed reader fails the test until it leaves the list. In `confirm-4d`, which burns the sealed confirm fold, every row-free refusal lands before the picks are frozen, and a refusal found in the rows withdraws the freeze.
