...
- **A scratch copy that runs the test suite needs `--with-git`.** The default omits `.git`, and four test files shell out to it — `emptyCorpusRefusals`, `feedSource`, `securityHardening` and `scratchClone` itself fail with `Command failed: git status --porcelain` and `git ls-files -z`, 19 failures that look nothing like a missing `.git`. `tests/scratchClone.test.ts` pins that set, so a fifth git-dependent test fails there and this line gets updated rather than the next agent rediscovering it. Measured: 8.2 GB working tree → 12 MB without git, ~171 MB with it, and the suite is 2734/0 from a copy holding no `.calibration-cache` at all.
