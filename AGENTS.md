...
- **Backups live in two buckets, and only one of them forgets.** Dailies go to `windwardline-backups`: each writer keeps 60, and the bucket's `expire-backups-after-365-days` rule deletes every object a year after upload whatever its name, so the minute bank's name-protection of `20260823` holds against this repo's prune and not against R2. What must outlive a year goes to `windwardline-archives` through `scripts/ops/push-archive-offbox.sh`: no expiry, write-once, an indefinite bucket lock once the first pushes verify, and pruned by no script — both pruners refuse any bucket whose first path segment is `windwardline-archives`, because rclone reads everything after `R2:` as bucket plus path and an exact-name refusal is walked past by one suffix, and `tests/archiveOffbox.test.ts` derives that from every file in `scripts/ops/`. The push delivers its own credential through `wl-secret` by absolute path, as its two scheduled siblings do, so the token is not in the launcher's environment. Under the lock the order is the guarantee: a new key's archive is extracted in staging and `diff -rq` against the source must be empty BEFORE `copyto --immutable`, then the object R2 returns must be byte-identical to what was sent, because a proof that failed after the upload would spend the key forever. An existing key is never rebuilt — `brew upgrade --formula` moves `tar` and `zstd` daily, so a rebuild proves nothing about the object — and is proven by streaming it back and restoring it against the source. Staging's free space is checked before the first write and again before the upload. Once a source is gone, the fleet cadence re-proves its archive: key and size weekly, md5 monthly. `rclone hashsum` on a multipart object reads metadata rclone wrote. Local data roots name a directory, never the home folder itself — `tests/opsDataRoot.test.ts` fails any ops script whose default lands there and names each dated exception it still carries. The register, the lock's state and the restore commands are in `docs/offbox-archives.md`.
