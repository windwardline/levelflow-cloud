...
- **The minute bank is proven by restoring it, weekly.** `scripts/ops/verify-minute-bank-restore.sh` restores the newest R2 archive and requires every restored file to be a byte prefix of the live bank; parity compares names and the push one md5, so neither is evidence that an archive restores.
