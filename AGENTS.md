...
- **Every behaviour-changing analyzer change bumps `ANALYZER_VERSION`** (`supabase/functions/trade-analyzer/calibration.ts`): it scopes global learning and names the physics a corpus was emitted under. The same change set places the new version in `PLACED_ENGINE_VERSIONS` (`scripts/banked-fraction.ts`) and extends that table's expected population in `tests/bankedFraction.test.ts`, or the test fails: the reader refuses a version it cannot place rather than guessing which exits a corpus already slipped.
...
- **The Edge refuses user-class provider spend while `DESK_PARKED` is true** (amendment 47; `supabase/functions/_shared/deskParking.ts`). `PARKING_GATE` turns away signed-out arrivals only, and a live session walks past it. Every Edge fetch site takes an `FmpSpendPermit`, asserts it before it builds a URL, and records its bytes to the permit's class. Only `mayFetch` mints a permit, once per request, and it fails closed on a ledger outage. `DESK_PARKED` implies `PARKING_GATE`: park by raising `PARKING_GATE` first (or both together), then §17p's logout; unpark by lowering `DESK_PARKED` alone, and lower `PARKING_GATE` only after that deploy's full E2E is green. Background work (news-calendar, outcome-sync) is not parked (§21i) and runs under its 200 MiB/day ceiling. `tests/fmpBudgetByClass.test.ts` holds this at fetch-site grain, deriving every site from the provider's identifiers rather than a URL's spelling; `scripts/deploy-e2e-scope.sh` reads `DESK_PARKED` before the diff, so a parked dispatch stands the FMP-spending projects down.
...
- **The minute bank is proven by restoring it.** `scripts/ops/verify-minute-bank-restore.sh` restores the newest R2 archive, refuses one more than three days old, and requires every restored file to be a byte prefix of the live bank and every live symbol older than the snapshot to be in it; parity compares names and the push one md5, so neither is evidence that an archive restores. It runs weekly beside the Postgres proof on the fleet-health cadence (windwardline#118); first proof 2026-09-22, 100 data files and 3,516,278 bars.
...
- **Backups live in two buckets, and only one of them forgets.** Dailies go to `windwardline-backups`: each writer keeps 60, and the bucket's `expire-backups-after-365-days` rule deletes every object a year after upload whatever its name, so the minute bank's name-protection of `20260823` holds against this repo's prune and not against R2. What must outlive a year goes to `windwardline-archives` through `scripts/ops/push-archive-offbox.sh`: no expiry, write-once, an indefinite bucket lock since 2026-09-22 (set after the first three pushes verified), and pruned by no script — both pruners strip `BUCKET`'s leading slashes as rclone does and refuse any bucket whose first path segment is `windwardline-archives`, and `tests/archiveOffbox.test.ts` reads every file in `scripts/ops/` as the shell would, fails closed on an rclone call it cannot place, and pins which files may write to R2. Outside `scripts/ops/`, no file git tracks or would commit may name the R2 capability — rclone, `RCLONE_CONFIG_`, `cloudflare-r2-backup`, the R2 endpoint, either bucket or `wrangler r2`. `tests/offboxCapability.test.ts` holds that census and exempts `AGENTS.md`, `docs/` and `tests/`, which name the capability without holding it, and the review lane's `.fleet-standard/`, which this repository never tracks. The push delivers its own credential through `wl-secret` by absolute path, as its two scheduled siblings do. Under the lock the order is the guarantee: a new key's archive is extracted in staging and `diff -rq` against the source must be empty BEFORE the upload, then the object R2 returns must be byte-identical to what was sent, because a proof that failed after the upload would spend the key forever. The upload is `copyto --ignore-existing` under a per-key lock; rclone ignores `--immutable` on `copyto` and replaced a different object with exit 0 (v1.75.1, 2026-09-22). An existing key is never rebuilt — zstd moves with `brew upgrade --formula` — and is proven by streaming it back and restoring it against the source; only a failure of the object's own bytes spends the key. On R2 an absent prefix lists empty with exit 0 and exit 3 is a missing bucket. Staging's free space is checked before the first write and again before the upload. Once a source is gone, the fleet cadence re-proves its archive: key and size weekly, md5 monthly (windwardline CADENCE.md, from windwardline#117). Local data roots name a directory, never the home folder itself — `tests/opsDataRoot.test.ts` fails any ops script whose default lands there and names each dated exception it still carries. The register, the lock's state and the restore commands are in `docs/offbox-archives.md`.
...
- **FMP script state is append-only, and the minute bank is never refused at a door (§21c).** `.fmp-state/` holds the consumer-keyed byte ledger, the breaker's event log and the run markers; every writer appends, and the legacy `.fmp-usage.json` and `.fmp-circuit.json` are read as history, never written. Governor entry points take state paths as a required argument, so a test cannot reach the live ledger by omission; the binaries resolve them through `scripts/checkoutState.ts`, like every other piece of ignored state. The top-up and ad-hoc classes each get 256 MiB a day from a pool that reserves 333,333,333 bytes for the bank; the bank bounds each run at 512 MiB instead. An ad-hoc run above 256 MiB needs the owner's `--daily-ceiling`.
- **A pinned job names its checkout, and no test reaches the provider.** Jobs run through `wl-repo-script` get `origin/main`'s code and none of the checkout's ignored data, so anything touching FMP must set `LEVELFLOW_CHECKOUT`: `.fmp-state/` (the byte ledger, the breaker log, the run gate's markers) and the legacy ledger and marker resolve through `scripts/checkoutState.ts`, and unset they read as untouched from a directory deleted on exit — for the run gate, a gate that finds no clean run and re-runs the job at every login. The minute bank and the calibration-cache top-up have run this way since 2026-09-20; `docs/minute-bank.md` lists what each piece of the bank's data would otherwise have done, and the top-up adds a fourth: the sweep's cache is the relative `.calibration-cache`, so an unnamed run warms the roster from nothing into a directory deleted on exit. Its script passes `--cache-dir` explicitly and refuses a missing one. A test that executes a script which reads the keychain runs with `tests/support/noKeychain.ts` in its environment. On 2026-09-20 six runs without it fetched a full FMP roster each, about 244 MB, two of them into the production bank: a red test executes the code before the guard it is written for exists.
- **Every argv reader refuses an unknown flag by name, and the population is derived.** `tests/unknownFlagRefused.test.ts` finds each reader by what it reads (sixteen spellings of an argv read, node:util's `parseArgs` among them, pinned), executes it with an unknown flag and a stray argument, and requires every flag it reads to be declared; the shared walk is `positionalArgs` and `flagsOnly` in `scripts/flagReader.ts`. `confirm-4d` and `derive-4d` resolve the markets they grade through one function, `resolveGradingPopulation` (`scripts/sweepFolds.ts`), before either writes: `--targets` beside `--holdout-cycle` refuses, a target off every shard's roster refuses, and an empty holdout draw refuses; a new grader resolves its population there, never inline. `STACK_ON_REFUSAL` names the twelve readers that still print a stack on an operator's typo, and it only shrinks: a fixed reader fails the test until it leaves the list. In `confirm-4d`, which burns the sealed confirm fold, every row-free refusal lands before the picks are frozen, and a refusal found in the rows withdraws the freeze; a withdrawal that itself fails is reported beside the refusal, never in its place, naming the file to fix by hand.
...
- **`--no-git` costs six test files.** `scripts/scratch-clone.sh` ships `.git` by default; `--no-git` opts out and is right for a copy that will not shell out to git. Six test files here do — `emptyCorpusRefusals`, `feedSource`, `securityHardening`, `scratchClone` itself, `researchLinks` (the citation guard asks git which files are tracked) and `offboxCapability` (the off-box census asks git what it tracks or would commit) — and without `.git` they fail with `Command failed: git status --porcelain` and `git ls-files -z`, 33 failures measured on 2026-09-22 (26 of them `emptyCorpusRefusals`, one per corpus reader) that read as missing DATA rather than a missing directory. Those failures are why the fleet inverted the default (windwardline#84). `tests/scratchClone.test.ts` pins the set, so a seventh git-dependent test fails there and this line gets updated rather than the next agent rediscovering it. Measured: 8.2 GB working tree → 171 MB by default, 12 MB with `--no-git`, and the suite is 4052/0 (one skip) from a default copy under `/private/tmp` holding no `.calibration-cache` at all (2026-09-21). A test that needs a real directory outside every temp root makes one under the home directory's `.cache`, never `process.cwd()`: from a copy under a temp root, the off-box push's sandbox guard fired before the refusal the test was written for.
