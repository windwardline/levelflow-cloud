...
Of 91 markets: ~~80 judged and failed, 11 no verdict~~ **69 judged and failed,
22 no verdict, 0 accepted.**

**Corrected 2026-09-14** ([`empty-fold-verdicts-2026-09-14.md`](/docs/research/empty-fold-verdicts-2026-09-14.md)):
the gate reported a market with NO fit-fold rows as a measured failure, because
an absent fold differences to 0 − 0 and no leg of the no-verdict ladder tested
for emptiness. Sixteen graded markets have no fit fold — eleven crypto and five
others (ASX, DAX, DOW, FILUSD, NIKKEI) — and eleven of them sat in the failed
bucket here and should have been no-verdict. **The verdict is
unchanged — 0 of 91 accept, and the thirteen that beat the baseline are the same
thirteen** — because an empty fold cannot produce an acceptance either way. The
artifacts below were regenerated by re-running this note's own command.

**69 were actually tested, not 88.** THREE reasons take a market out of the
tested population; the original text named one and a first correction named two.
(a) On NSDQ and SP every decision already falls inside 16:00–21:59 UTC, so the
predicate retains 100 % of their fills and cannot differ from the baseline at
all: `ΔR fit 0.0`, `ΔR select 0.0`, and 0 of 148 and 0 of 146 nonzero pairs.
(b) Eleven markets have no fit fold to test against. (c) The remaining nine are
underpowered or carry an unresolvable pairing — and several of those are markets
where the rule plainly did differ, so they are not no-ops. Together, 22 markets
return no verdict and 69 carry one. "0 of 91" should not be read as 91 rules
tested, and the number that were is 69.
