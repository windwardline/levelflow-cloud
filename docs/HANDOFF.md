...
10. **Standing:** §21c's premise in light of E3; which vintage research reads (neither is clean; priced by the [vintage bound](/docs/research/vintage-bound-2026-09-24.md): on forex, near the shipped stop width, about 0.01 R per bracket against the bank's copies, 95 % about 0.001 to 0.019 clustered by day over 14 days (the clustering is JUDGED; the direction holds on sign tests over days and over pairs, 9 of 9 and 14 of 16, and not per bracket): the size of the forex edge); `confirm-4d`'s `--baseline` default; refresh the FMP population attestation (expired 2026-09-15); read the FMP dashboard and say whether `--daily-ceiling` needs a dated label; the FMP key rotation is not owed for the thirteen 2026-08-07 rows but stays open, because Edge console logs since 2026-08-18 would carry the current key and are unmeasured (FMP plan E4); empty the Trash; revoke the Gemini CLI's Google OAuth grant.
