...
- The statistical core is byte-identical
  to `d0b9907` across `familyPairedP`, `permutationPValue`, `mulberry32`,
  `supportOf`, `MIN_EFFECTIVE_PAIRS` and the `accepted`/`thin`
  expressions — re-verify this on every change to `grid-totalr.ts`.
...
itself draws on. Twelve rounds ran on #364. They earned their cost (they
