...
3. **The vintage bound** (converge rank 4): a zero-byte differential on synthetic brackets over 2026-08-04..08-25 and the 25 construction-clean symbols, first copies against revised history; record the band.
...
7. **Leftovers** (rank 13): ~~the Donate route while parked~~ (landed 2026-09-24, #705: the parking screen answers its own Donate); detecting an orphaned `confirm-4d` freeze; the rebuild logs pushed off-box; the recovery's scan speed; the donation block `AuthScreen.tsx` and `ParkingScreen.tsx` both render, identical but for its wrapper (share it, as `satelliteFrame.ts` shares the frame).
