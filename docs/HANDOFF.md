...
in `windwardline-archives` through `scripts/ops/push-archive-offbox.sh`. Pending the
operator's run; the register is `docs/offbox-archives.md`. The `~/` paths below are the
