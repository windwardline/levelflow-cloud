...
**Landed, merged, deployed where that applies (deploy runs green through `32393b6`).**
- **#689** and **#692** — every market-order exit now prints with the modelled slippage against the position: a stop that did not gap (`2026.09.23.stop-exit-slippage`) and the review-end close (`2026.09.23.expiry-exit-slippage`, live). Before, the slippage the admission gate charges reached realized R only on gapped opens. Repriced without a re-simulate by `scripts/banked-fraction.ts --exit-slippage`, fit and select only, confirm sealed ([record](/docs/research/r3/exit-slippage-2026-09-23.txt)): on R3's corpus of record forex select +1,603.8 → +1,009.3 R and forex fit −94.7 → −1,233.1 R (0.0075 and 0.0065 R per fill), pooled select −5,680.1 → −7,070.7 R. On the 2026-09-14 corpus forex fit +440.1 → −565.9 R. **Every expectancy printed before 2026-09-23 is optimistic, by 0.006 to 0.011 R per fill on these folds.** **#695** makes the reader's printed statements true on every class and scale and places engine versions from an explicit table: every future `ANALYZER_VERSION` bump adds its line to `PLACED_ENGINE_VERSIONS` and to that table's expected population in `tests/bankedFraction.test.ts`, or CI goes red (AGENTS.md).
- **#694** — `scripts/entry-excursion-screen.ts`, amendment 46's random-entry screen, controls first, zero bytes. On the 2026-09-14 corpus's fit fold (W = 8 h, 243,530 rows, confirm withheld at the door): look-ahead passes in 51 of 51 markets with a verdict, the coin flip in 0 of 51, and the **shipped entry in 0 of 51** (forex pooled +0.0238 ATR, t = +1.22). The shipped entry's interval excludes zero in ten of 51 markets and none clears its cost floor. The markets share legs, so that count is not 51 independent tests: three of the eight above zero are JPY crosses (AUDJPY, CADJPY, USDJPY), and five of six JPY crosses read above zero ([record](/docs/research/r3/entry-excursion-controls-2026-09-22.txt)). It takes no family input: that waits for a registry. Exit 3 is a control that failed; 4 is a read it could not examine (a market not pinned or placed, a row that did not anchor, a censoring sample with nothing attempted); 2 is nothing to screen.
- **#687** — the low-edge refusals state their window and cite no measurement: every low-edge hour was set on the pre-repair clock and none has been re-derived. The hours stand; a market-grain re-grade rides the next re-simulate. The Target 2 refusal names its figure instead of calling it payoff.
- **#686**, **#690** — `verify-fmp-matches` starts its budget, fetch and state unset behind one accessor, and an ungoverned spend is `UngovernedSpendError`, final and with a stand-down token; `replay-sweep`'s two pre-`main()` guards throw it too. A derived pin covers every FMP script's module scope.
- **#691** — the guard populations are derived and recorded, not counted: argv readers, door readers (28 with the screen), the outcome columns, `STACK_ON_REFUSAL` (12), the temp-root census, and a new off-box census (`tests/offboxCapability.test.ts`: nothing outside `scripts/ops/` names the R2 capability). `--no-git` now costs six test files, 33 failures.
- **#693** — the citation guard reads every tracked text file (275 citations in 123 files), joins wrapped paths, and the low-edge source scan reads interpolated helpers.
- **#688**, **#696** — the regrade design, the FMP plan and memo, and amendment 48's parked records are in `docs/research/designs/`.
- This change set: the state of record below.

**Amendment 48.** Parked, not law, and absent from the owner-rulings specs. Its reconciled draft (the last section of [the record](/docs/research/designs/amendment-46-parameters-2026-09-21.md)) took its first independent refute round: four lensed refuters, each checked; 49 findings, 4 overturned; **six majors survive their checkers**, and a seventh, scoped to minor, is kept among them because it is what parked the last pass ([round](/docs/research/designs/amendment-48-refute-round-2026-09-23.md)). L is a free parameter the seal cannot see; a family registration hashes no trade; the span rule changes 46's premise without the owner; the frontier is per symbol while four correlation groups cross classes; two documented routes (`grid-totalr --confirm-log-dir`, `replay-sweep --print-confirm-table`) open a confirm fold with no record once frontier-anchored spans exist; the interim sweep ban has no mechanism; the section's extent is undefined. Each carries its checker-corrected fix.

**Machine state.** Every worktree from tonight is removed and the shared checkout is on `main`, clean. `docs/amendment-48` stays on origin as drafting history, not to be merged: it adds parked text to the binding specs file.
...
3. **The vintage bound** (converge rank 4): **measured 2026-09-24** ([record](/docs/research/vintage-bound-2026-09-24.md)). Across 91 symbols 5.6–6.0 % of synthetic brackets flip between the bank's first copies and FMP's later history, net against the bank; forex is the worst class: the vintage moves outcomes against the bank's copies at every width (every day with a difference negative at the widest, 9 of 9), and at the width nearest the shipped stop by −0.010 R per bracket, 95 % about −0.019 to −0.001 clustered by day over 14 days (the clustering is JUDGED; the direction rests on sign tests over decisions, pairs and days, p = 0.0007, 0.004 and 0.004, and per bracket does not reject, p = 0.33); a forex figure that moves by less than about 0.02 R per bracket, the far end of that day-clustered interval, is a vintage question, not a finding (about 0.018 in shipped-stop R, a forex bracket at k = 2 being about 0.9 of a shipped stop; under the per-bracket estimator the line would be 0.03 R per bracket); futures is not established, moving against the bank's copies at the tightest width (−0.039 R per bracket) under three of four clusterings but not by symbol, its widest, with HOUSD carrying about half, and no futures line is sized: at k = 0.5, where the −0.039 is measured, a futures bracket stands for 3.5 % to 29 % of its market's shipped stop (forex's for 20 % to 24 %), and HOUSD and RBUSD, which carry most of the net, are the two narrowest, so futures' R per bracket is not in shipped-stop R; crypto and metals are not measurably moved at any width; no constant offset lies behind the closes, but the bank's bucket sits lower at both extremes in 23 of 28 forex pairs, and futures' sits inside the cache's at both ends, so on neither class can the record separate revision from construction. The "25 construction-clean symbols" could not be reproduced (the definition died with the 2026-09-23 session) and that filter reads almost nothing, so the record reports the whole roster.
4. **One re-simulate** (rank 11), zero bytes from the anchor-pinned 2026-08-26 slice, capture-all plus an `--ignore-low-edge` arm, to new paths. List the version bumps and expected differences first (both slippage versions). Then re-grade on fit and select only: the eight arming-bound cells, `grid-totalr` per market, `banked-fraction`, and the low-edge windows at market grain on net R.
5. **The cross-rate parity instrument** (rank 12), with a reader exclusion for the recovered EURUSD block.
6. **Ops** (rank 9's remainder): run both restore proofs through `wl-repo-script` in windwardline CADENCE.md; document `--spend-class` and fix the `fmpCircuit.ts` label; a handoff step that triages every merged PR's late review comment; `update-types` on the npm security group as a fleet change (FLEET.md, checker, template, every repo).
7. **Leftovers** (rank 13): the Donate route while parked; detecting an orphaned `confirm-4d` freeze; the rebuild logs pushed off-box; the recovery's scan speed.
8. **Bookkeeping** (rank 14): the regrade build and the FMP records change set, both designed in `docs/research/designs/`; `fmp-entitlements.md`'s "nine endpoints" rebuilt from code; the 1,417 against 805 "Invalid API KEY" count; the COT top-up gap; `buildScanRationale`'s unrendered "Payoff" string.

**Carried from tonight's reviews, advisory, none red today.**
- The low-edge source scan (`tests/sessionContext.test.ts`): a reason that calls a helper without interpolating it, an interpolation inside a helper's own literal, and an imported callee are each read by nobody. Anchor each reason as one string or template literal, apply the one-call rule inside helper literals, and treat an imported name as a binding the scan cannot read.
- `scripts/banked-fraction.ts`: the slippage-priced table charges the raw slippage, not the corpus's `modeledCostScale`; it now says so rather than changing its figures.
- The entry-excursion screen: a persistent-side placebo (one coin per market-day) is the control that could speak to the interval's calibration, which the per-decision coin flip cannot; the null matches a decision's clock but not its realised window, so a Friday decision clamped by the weekly close is compared with nulls that were not (print the two window distributions per market, or draw same-weekday nulls); the null's own estimation error is subtracted as exact; the censoring control has no coverage floor (one attempted row of 2,000 drawn reads HOLDS at exit 0), so state a minimum attempted share beside `CENSOR_SAMPLE`; and a re-resolution truncated by a series that ends or has a hole before the exit is counted as a mismatch rather than as a row not attempted.
...
10. **Standing:** §21c's premise in light of E3; which vintage research reads (neither is clean; priced by the [vintage bound](/docs/research/vintage-bound-2026-09-24.md) class by class: on forex, near the shipped stop width, about 0.01 R per bracket against the bank's copies (a forex bracket at k = 2 is about 0.9 of a shipped stop), 95 % about 0.001 to 0.019 clustered by day over 14 days (the clustering is JUDGED; the direction holds on sign tests over decisions, pairs and days, 31 of 40, 14 of 16 and 9 of 9, and not per bracket), comparable in magnitude to the forex edge per fill, though a bracket is not a fill; futures not established, −0.039 R per bracket at the tightest width under three of four clusterings but not by symbol, in a mixed unit that is not shipped-stop R and with no line sized for it; crypto and metals not measurably moved at any width; on forex and futures the record cannot separate revision from construction); `confirm-4d`'s `--baseline` default; refresh the FMP population attestation (expired 2026-09-15); read the FMP dashboard and say whether `--daily-ceiling` needs a dated label; the FMP key rotation is not owed for the thirteen 2026-08-07 rows but stays open, because Edge console logs since 2026-08-18 would carry the current key and are unmeasured (FMP plan E4); empty the Trash; revoke the Gemini CLI's Google OAuth grant.
...
2. **Amendment 48** is parked, and the redesign is this repo's to do. The adopted sections disagree with each other (filters carry no readiness floor; the cluster and df rules differ), calibration programs have no rule, the settled effect floor conflicts with the random-entry screen's ATR unit, and the adopted sections never took a refute round of their own ([check round](/docs/research/designs/amendment-48-check-round-2026-09-22.md)). *(2026-09-23: redesigned and refuted once; six majors stand, plus one kept — see the 2026-09-23 entry.)*
...
**Owner only.** *(Superseded by the 2026-09-23 list above; the ambiguous-fills item is answered there.)*
...
6. **Amendment 48:** record amendment 46's parameters as law under the standing approval — design record [`amendment-46-parameters-2026-09-21.md`](/docs/research/designs/amendment-46-parameters-2026-09-21.md), on main since 2026-09-23 — stating the per-market reading of "one capped total alpha" as the reading taken.
...
fact, measured 2026-09-22. Once the store spans a window it re-asks its newest
stored date to today, where a revision supersedes the stored copy, and an older
settled bar is not asked for; a request reaching past the store's oldest date
re-buys the whole window, and its revisions land then.) `market_bars` holds RAW provider rows and `fetchFmpBars` buys only
...
*2026-09-21:* the backup's snapshots leave the home folder. The script had
defaulted its root to `/Users/peacock` and kept fourteen dailies there, so
fifteen `levelflow-minute-bank-snapshot-*` directories sat beside the owner's
files. The root is now `~/.local/share/levelflow-cloud/minute-bank-snapshots/`
and one daily is kept beside the protected `20260823`: R2 holds 60 verified
...
low-edge gates stay the deliberate, stated asymmetry (2026-09-23: the hours
stand, but every one was set on the pre-repair clock and none has been
re-derived; since #687 the refusal states only its window, and a market-grain
re-grade of the hours is queued with the next re-simulate). 1f-c folds in: the
...
| 4 | ~~**Two of Phase 4's five axes cannot be expressed as grid overrides** (AXES-3 pivot depth, AXES-9 oscillatorBias)~~ **CLOSED AS WRITTEN 2026-09-01 (#539, #540) — and both halves of the entry were wrong.** Both named axes are now grid overrides: `pivotStrengthIntraday` / `pivotStrengthDaily` replace the `findSwingPivots(bars, 3)` / `(daily, 2)` literals, and `rsiBuyThreshold` / `rsiSellThreshold` replace the 55/45 in `resolveOscillatorBias`. Every default is the shipped literal, verified bit-identical on a real anchored corpus, so no `ANALYZER_VERSION` bump and no behaviour change. **Wiring them exposed a defect the entry could not have anticipated**: `runStrategyCommittee` looked its calibration up itself, so ANY voter-side grid axis was structurally inert — the sweep's override never reached a voter. That is fixed and covered behaviourally at both the committee and the sweep. **The enumeration was short**: `blockedRegimes` (AXES-4) and `sideScoreAdjustments` (AXES-5) are equally unexpressible, and they are not an oversight of the same kind — `GRID_OVERRIDE_KEYS` mirrors the calibration's NUMERIC fields, and these two are an array and a record, so they need a structured-value grid mechanism that does not exist. **And the stated consequence is overstated.** "R3's one re-sweep cannot produce the corpus R4 is defined to read" was written when a re-sweep cost provider bytes. Item 2's law is *one re-simulate after the INSTRUMENT changes, never one per fix* — a rule about corpus coherence under one engine, not about process count. At a pinned anchor a sweep spends nothing, so R4 may add arms at the same anchor for free provided `analyzerVersion` is unchanged, and it is a term in `conditionsOf` so a corpus cannot silently mix engines. What must land before the engine next changes is EXPRESSIBILITY, which is what this closes | **CLOSED for its two named axes; AXES-4 and AXES-5 remain, and are R4's to design** |
...
- **The arming-bound arm (2026-09-14, after the open-scope round).** The
  sweep resolves every decision a THIRD time with FR-3's same-bar arming off
  and emits `armingBoundRealizedR`, `armingBoundOutcome`,
  `armingBoundExitPrice` and `armingBoundExitAtMs` beside the net and gross
  arms — the one-bar-latency bound (the refuters' counterfactual: the stop
  arms one 5-minute bar later, which removed +4,342 of the lock's +4,689 R on
  fit in-pool), not the hold-priced bound the round's note tabulates.
  Identical to the net arm on every row whose TP1 bar did not close back
  through the armed level. The corpus of record predates the column: the one
  re-simulate carries it.

None of these columns costs a provider byte; the values were already computed
and discarded. **A packet figure was wrong and is corrected in place**: question 3
...
verifiers — true of this corpus; since #689, `2026.09.23.stop-exit-slippage`,
the engine prints every clean stop with the modelled slippage); 60 % of forex lock exits resolve on the touch bar (FR-3), which
...
touches the stop. **ON FOREX THE STOP IS MOSTLY STRUCTURAL — two earlier
versions of this block said the cap binds on every row and both were wrong.**
Measured, `forex-stop-provenance-2026-09-12.txt`: over 373,510 forex baseline
...
the swap, +54.5 R against booked money, three quarters of it AUDCAD and
AUDNZD, in-sample and deciding nothing; the eleven EUR/GBP-base crosses only
admit, AUDJPY/CADJPY/NZDCAD/NZDCHF/NZDJPY only decline, CHFJPY barely moves;
the kept rows' money moves +158.4 R two-sided. The payoff-floor channel the
reader does not measure is inert by a calibration inequality (the record says
why); an independent refuter reproduced all 66 cells with zero differences. The corpus of record and the 2026-09-14 re-simulate carry
...
**32.6 % of decisions IN BREAK-CARRYING CLASSES hold a review window running
through hours the market is shut** (crypto trades continuously, carries no
break, and is excluded) — forex 33.0 % (123,321 of 373,510), livestock 100 %,
indices 67.7 %, agriculture 43.2 %, metals 33.2 %, futures 18.4 %, energies
14.7 %. Population: baseline variant, accepted, all graded folds, holdout
included, confirm byte-skipped. Livestock's 100 % is structural — a 24h window
crosses any daily break. The round's
...
clock. **Break lengths differ and the difference matters**: livestock runs on the
CME complex's ONE-HOUR break (`calendarFor`, `marketHours.ts:109-118`), so it
loses ≈4.2 % of its 24h window — the 5h40m grains break belongs to AGRICULTURE.
Measuring the exposure was free; REPAIRING it is not — it means deciding what
...
clock defect condemned — and its own note said the entries stood on the
conservative reading only, to be re-decided the moment a valid corpus exists.
`scripts/register-verdict.ts` now derives every entry from the one ledgered
confirm read into `docs/research/r4/withdrawal-verdict-2026-09-03.json`, under
a rule hashed into the artifact.

**What changed.** Ten markets enter (ALGOUSD, ASX, ATOMUSD, AVAXUSD, BCHUSD,
DOTUSD, DYDXUSD, NEARUSD, SOLUSD, TRXUSD) — together **−5,000R over 9,863
confirmation-fold fills**. Thirteen stay on re-based evidence. **Two are restored**:
...
### ▶ RESUME HERE — 2026-09-03 19:30 UTC (the read's calibration change; superseded by the block above)
...
- **Every class has a class-grain reading now.** The card's global folds put
  ...
  docs/research/r3/fold-spec-2026-08-26.json`, both acceptance modes, zero
  bytes, 2h29m / 2h32m, record §10): no grid variant is accepted in any of
  the eight classes; futures −0.101, agriculture −0.135, indices −0.167,
  energies −0.201, livestock −0.205 at rest on fit+select, and four of those
  five are negative on the GROSS arm too. Two corpora now exist at one anchor
