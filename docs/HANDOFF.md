...
**Rounds 49–53 closed the rest of that audit's population.** Three of the
five consumers — `threshold-rescue`, `cost-sensitivity-verdict`,
`feasibility-4d` — carried **no empty-corpus door at all**, where
`roster-expectancy-audit` and `market-dossier` each carried one. Run with
no corpus, each wrote an artifact that looked like a finished run:
