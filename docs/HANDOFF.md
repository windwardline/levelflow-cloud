...
| 5 | **Amendment 36's re-decision needs a gross corpus AND a net corpus**, and `LEVELFLOW_MODELED_COST_SCALE` is a per-process env read, so `--grid` cannot produce both arms in one run | **STILL OPEN after M5 (2026-08-31)** — M5 made the knob real, not cheap. Two runs where the sequence budgets one, against an exhausted allowance. **Recommendation: emit BOTH arms from one run.** The resolver is pure given (bars, plan, options), so resolving each decision a second time at a different scale is CPU and nothing else — zero additional FMP bytes, against a second full sweep's worth. Costs an emit column naming the arm and a manifest term; the alternative costs the bandwidth twice |
...
instrument (D4 the gate has NO absolute-expectancy term — DONE 2026-08-31, M3 the confirm
read decides on a bare delta with no error bar, M1 the audit
double-counts, M5 make the cost scale actually reach the resolver — DONE
