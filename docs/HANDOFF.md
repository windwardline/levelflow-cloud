...
| **R0f** | **THREE OF SIX INDICES ARE ON THE WRONG CLOCK — R0's "one clock" is NOT achieved.** NEW 2026-08-24, found by chasing a store whose last bar was stamped in the future. FMP labels foreign index bars in **local exchange time**; the normalizer reads every label as New York wall. Each foreign index is therefore displaced by exactly its local-to-New-York difference: **^GDAXI +6h, ^N225 +13h, ^AXJO +14h**, against 0h for ^GSPC/^DJI/^NDX. Confirmed FOUR independent ways: the offsets equal the venue differences exactly; ^N225's hourly histogram dips at raw label 12:00, the Tokyo lunch break; each index observes ITS OWN exchange holidays (^AXJO absent 2025-01-27 Australia Day and 2025-04-25 ANZAC Day, present 2025-07-04; ^N225 absent 2025-05-05 Golden Week, present on US Thanksgiving); and the repo's own `sessionAnchorWitness` convicts all three — displaced, 4 displaced years each, 0 anchored — while the three US indices read anchored with 0 displaced. **Nothing caught it because every relative instrument is blind here BY CONSTRUCTION and the docs already said so**: the transition witness needs 24/7 coverage so sessioned indices read `indeterminate`; cross-series registration compares a symbol's 5min against its own 15min, which are displaced identically, so it reads `aligned`. The one ABSOLUTE intraday instrument is the session anchor, and `REFERENCE_SESSION_ANCHORS` held **`^GSPC` alone** — a hand-picked list of one guarding a failure afflicting six, which is FLEET.md's named archetype verbatim. The measured population is exactly these three: every other sessioned market sits where it belongs (livestock 13-18 UTC = CME 08:30 CT; futures/metals/energies 00-23 globex; agriculture 00-18/19 CBOT). **DETECTION SHIPPED 2026-08-24**: anchors are now expressed in the VENUE'S OWN IANA zone rather than a New York wall hour — Tokyo keeps no DST and Sydney's runs the opposite way, so a wall-hour anchor left Tokyo marginal against the witness's 0.6 modal floor and Sydney structurally unresolvable, and an unresolved anchor reads INDETERMINATE, which the verifier fails. The population is now DERIVED: `tests/clockWitness.test.ts` walks `ASSET_TYPE_BY_SYMBOL.indices` and fails if any index lacks an anchor, and a second test pins each index's venue so a wrong zone cannot silently flip a verdict (that mutation survived the first four and is now caught). **CORRECTION SHIPPED 2026-08-24.** `supabase/functions/trade-analyzer/venues.ts` is the one place a market's exchange clock is recorded — zone AND session open together, so the anchor witness verifies the same fact the normalizer reads and a wrong zone is a loud failure rather than a silent mis-registration. `toTimestamp` and `normalizeFmpBars` now take a REQUIRED zone (no default, because a silent New York default is the defect itself), threaded from `labelZoneFor(providerSymbol)` on both the live path (`marketLoader.ts`) and the offline one (`replay-sweep.ts`). `REFERENCE_SESSION_ANCHORS` is now derived from `VENUE_CLOCKS` rather than duplicating it. `BAR_CLOCK` is **`venue-wall-utc-v3`** — renamed because the clock is no longer New York's. Verified per venue in both DST regimes: ^N225 00:00Z in January AND July (Tokyo keeps no DST), ^GDAXI 07:00Z under CEST and 08:00Z under CET, ^AXJO 00:00Z under AEST and **2026-01-14T23:00Z under AEDT — the previous UTC day**, which is the case a New York wall-hour anchor could never express. The other 93 sources are unchanged: their venue IS New York, so v3 assigns them the instants v2 did. A per-zone memo was required in the same change: the conversion cache was `Map<number, number>`, so leaving it flat would have returned a New York answer for a Tokyo stamp, silently, for any digits another symbol converted first | **BLOCKS R3.** Those three markets would be calibrated on data 6, 13 and 14 hours out of register — the same look-ahead mechanism that invalidated the 4c/4d corpus — and the macro join, calendar join, session gating and completion conventions all read against the displaced stamps. The normalizer is SHARED with the live analyzer, so this is not cache-only; the desk being parked is the only reason nothing is being served wrong. Step 3 now REDs these three by design: that is the new guard working, not a regression |
...
| **R0i** | **THE EMIT MUST CARRY A REJECTION LEDGER, AND R3 IS THE ONLY CHANCE — NEW 2026-08-24.** `rejections` in `simulateSymbol` (`sweep.ts:374-390`) is a COUNTER STRUCT, not a ledger: a decision that is rejected emits no row. That makes recoverability track DIRECTION, not effort — a divergence where the sweep is more PERMISSIVE than live leaves rows that can be found and pruned, while one where the sweep is more RESTRICTIVE leaves nothing but an incremented integer. Four of the eleven measured divergences are sweep-restrictive (`429` daily visibility, `442` five-minute visibility, `468` session block flips, `483` news window), so their populations are **unmeasurable from the corpus today**. The enumeration (`docs/research/decision-instant-enumeration-2026-08-24.md` §3) lists seven fields marked **cheap now / IMPOSSIBLE LATER**: the rejection ledger; the plan prices `limitEntry`/`stopLoss`/`takeProfit`/`takeProfit1` (verified absent — `legs` recovers entry for FILLED rows only); per-frame tail instants plus `availableTimeframeCount`; `decisionAtMs`; `dailyVisibleCount` + `dailyTailCompleteAtMs`; the realized news sets; and `symbolTailMs`. **The 5-minute finding is the sharpest and has no defensible reading**: the decision bar's three 5-minute children all close by the bar's own close, live holds 3–5 of them, and the sweep admits ONE — under the sweep's nominal clock that one has not yet closed (a 5-minute look-ahead), and under the honest clock it is a 10-minute shortfall. FMP's allowance is a rolling 30-day budget already exhausted once by replay sweeps, so "add the field in R4" is not a real option | **BLOCKS R3, and it is the largest single gap in the corpus.** Land with R2's implementation pass. Note what R3 CANNOT record and must never be claimed as closed: live's scan phase δ (0–15 min of uncertainty on every magnitude), the historical quoted spread, and FMP's Treasury publication minute |
