...
| **R0b** | **Back up both irreplaceable stores** — NEW 2026-08-23, and it was ranked nowhere. `.minute-bank/` holds 1-minute bars the provider will not serve again (the 08-13 to 08-18 gap is the precedent for what that costs), and the rebuilt `.calibration-cache` costs ~14 hours and metered bandwidth to reproduce. `scripts/ops/` contains seven files and none of them backs anything up; the only "backup" string under `scripts/` is an unrelated comment. HANDOFF recorded the gap once, in a parenthetical under the RUNNING block, and never ranked it — and **R0 step 5 deletes the condemned archive**, leaving the rebuilt cache as a single copy | **after R0 step 3 goes green, before R0 step 5 deletes the archive.** Offline and cheap; its window of maximum value is the moment the rebuild completes |
...
- **There is no reopen gate.** `PARKING_GATE` is a single boolean
  (`src/lib/parkingGate.ts`) whose only guard asserts it is `true`
  (`tests/parkingGate.test.ts`). Nothing ties reopening to R6, or to any
  measurement being valid. Meanwhile the money-positive rates from the
  **invalidated** corpus still ship behind a caveat. Amendment 34 says the Guide
  tells the measured truth; a caveat on a figure the program has condemned is
  not that. **This is pre-reopen work and it is currently homeless.**
