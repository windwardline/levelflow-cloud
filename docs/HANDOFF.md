...
3. **The vintage bound** (converge rank 4): **measured 2026-09-24** ([record](/docs/research/vintage-bound-2026-09-24.md)). Across 91 symbols 5.6–6.0 % of synthetic brackets flip between the bank's first copies and FMP's later history, net against the bank; forex is the worst class, −0.010 R per bracket at the widest bracket (95 % interval about −0.019 to −0.001 over 14 days), the size of its measured edge; no constant offset lies behind the closes, but the bank's bucket sits lower at both extremes in 23 of 28 pairs, so the record cannot separate revision from construction. The "25 construction-clean symbols" could not be reproduced (the definition died with the 2026-09-23 session) and that filter reads almost nothing, so the record reports the whole roster.
...
10. **Standing:** §21c's premise in light of E3; which vintage research reads (neither is clean; priced by the [vintage bound](/docs/research/vintage-bound-2026-09-24.md): about 0.010 R per bracket on forex); `confirm-4d`'s `--baseline` default; refresh the FMP population attestation (expired 2026-09-15); read the FMP dashboard and say whether `--daily-ceiling` needs a dated label; the FMP key rotation is not owed for the thirteen 2026-08-07 rows but stays open, because Edge console logs since 2026-08-18 would carry the current key and are unmeasured (FMP plan E4); empty the Trash; revoke the Gemini CLI's Google OAuth grant.
