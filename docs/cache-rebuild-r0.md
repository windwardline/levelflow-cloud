...
- **The allowance recovered early.** The owner purchased a **100 GB plan
  ...
  **200** — no 429. If this runbook is being executed much later, re-run
  one cheap probe first, through the governor: it asks the breaker and the
  ad-hoc class's day before its one request, and records the bytes to the
  ledger. The key reaches the process at exec and never argv:

  ```sh
  wl-secret fmp-api-key=FMP_API_KEY -- \
...
FMP_API_KEY="$(security find-generic-password -a peacock -s fmp-api-key -w)" \
  ...
  --byte-budget 30gb --daily-ceiling 30gb 2>&1 | tee ~/.local/share/levelflow-cloud/archives/levelflow-rebuild-$(date +%Y%m%d).log
```
