...
- Advisory analyzer: Supabase Edge Function `trade-analyzer`, using daily/intraday bars, a multi-strategy committee, news blackout records, session guardrails, correlation filtering, limit-order-only outputs, and RLS-owned inserts.
...
- Asset access: only verified categories are visible in the app. Restricted or unverified categories remain hidden until data quality is confirmed.
...
- Trade execution is outside the active product scope. LevelFlow remains an advisory market-review product.
